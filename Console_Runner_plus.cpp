...
void gameLoop(char& key_prs)
{
	...
	duration f_time = chrono::milliseconds(75); // about 13 FPS
	//chrono::duration<int,std::milli> f_time = chrono::milliseconds(34); // about 29-30 FPS
	//chrono::duration<int,std::milli> f_time = chrono::milliseconds(16); // about 60 FPS
	duration update_time = duration(30); //= chrono::milliseconds(34);
	duration render_time = duration(30); //= chrono::milliseconds(34);
	...
	while(key_prs != 'r' && key_prs != quit_key)
	{
		...
		time_p update_end = chrono::system_clock::now();
		
		/*   RENDER   */
		resetCursor();
		cout << "Frame took " << update_time.count() << " ms to update, " <<
		render_time.count()/1000000 << " ms to render!" << endl;
		//dog.DisplayStats();
		drawMap();
		/*   END RENDER   */
		...
		update_time = update_end - start;
		render_time = end - update_end;
